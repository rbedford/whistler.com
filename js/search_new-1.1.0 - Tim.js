$(document).ready(function () {
	$("#widget input:checkbox, #widget input:radio, #widget input:text, #widget textarea, #widget select, #widget input:submit").uniform();
});
...
function search_start_new() {
	...
	$(".datepicker").datepicker({
		...
		onSelect: function (selectedDate) {
			calculate_num_days($(this).parents(".booking").attr("id"), selectedDate);
		}
	});
	...
	/* tab specific departure dates */
	$("#booking_accom .arrival").datepicker().val(add_to_today(1).toString()).trigger('change');
	$("#booking_accom .departure").datepicker().val(add_to_today(3).toString()).trigger('change');
	
	$("#booking_transport .arrival").datepicker().val(add_to_today(2).toString()).trigger('change');
	$("#booking_transport .departure").datepicker().val(add_to_today(2).toString()).trigger('change');
	
	$("#booking_airport .arrival").datepicker().val(add_to_today(2).toString()).trigger('change');
	$("#booking_airport .departure").datepicker().val(add_to_today(4).toString()).trigger('change');

	$("#booking_activity .arrival").datepicker().val(add_to_today(2).toString()).trigger('change');
	$("#booking_activity .departure").datepicker().val(add_to_today(2).toString()).trigger('change');

	$("#booking_rental .arrival").datepicker().val(add_to_today(4).toString()).trigger('change');
	$("#booking_rental .departure").datepicker().val(add_to_today(4).toString()).trigger('change');
	
	calculate_num_days($(".booking:first").attr("id"));
	
}

function calculate_num_days(parent, selectedDate) {
	
	var parent_id;
	(!parent) ? parent_id = "#widget " : parent_id = "#" + parent + " ";
	
	var start_date = new Date(), 
		end_date = new Date(), 
		nights = parseInt($(parent_id + " #nights").val()), 
	...
	if (nights < 1) {
		$(parent_id + "#nights").val("1");
		calculate_departure_date(parent_id);
	} else if (isNaN(nights)) {
		$(parent_id + "#nights").val("");
	} else {
		$(parent_id + "#nights").val(nights);
	}
...
function calculate_departure_date(parent_id) {
	...
	(!parent_id) ? parent_id = ".booking:first " : parent_id = parent_id + " ";
	
	var ms_per_day = (24 * 60 * 60 * 1000), 
		num_days = parseInt($(parent_id + " #nights").val()), 
		start_date = new Date(), 
		end_date = new Date();
...
function show_child_ages(child_count, parent, is_widget) {
	var parent_id = "#" + $(parent).attr("id") + " #childAll";
	...
	if(is_widget) {
		resize_widget("table.booking:first");
	}
}
...
function search_lodging(parent_id) {
	
	parent_id = "#" + parent_id + " ";
	
    var arrival = jQuery(parent_id + '.arrival').val(),
		path = window.location.pathname,
		dep = $(parent_id + '.departure').val(),
	...
	var mm1 = a.getMonth() + 1, 
		...
		yy1 = a.getFullYear(), 
		dep = mm1 + "/" + dd1 + "/" + yy1, 
		iChildCount = parseInt($(parent_id + '#children').val()), 
		url;
	...
	var params = [
		...
		[
			...
			["childagearray", create_child_array(iChildCount)]
		]
	];
...
function create_child_array(iChildCount) {
    var strChildArray = "";
...
function search_transfers(parent_id) {
	
	parent_id = "#" + parent_id + " ";
	
    // transport - supercatid 3
    var path = window.location.pathname, 
		arrival = $(parent_id + '#arrival1').val(), 
		cat_id = $(parent_id + '#selectTransport').val(), 
		code = $(parent_id + '#selectTransport option:selected').text(), 
		url, 
		dep = $(parent_id + '#departure1').val();
...
function search_flights(parent_id) {
	...
	if ($(parent_id + '#hidAirport').val() == "") {
        ...
        var path = window.location.pathname, 
			airname = $(parent_id + '#adep').val(), 
			arr = $(parent_id + '#arrival2').val(), 
			dep = $(parent_id + '#departure2').val(), 
			air1 = $(parent_id + '#hidAirport').val(), 
			air2 = $(parent_id + '#aarr').val(), 
			adults = $(parent_id + '#aadult').val(), 
			child = $(parent_id + '#achild').val(), 
			url;
...
function search_activities(parent_id) {
	
	parent_id = "#" + parent_id + " ";
	
    // activity - supercatid 2
    var arrival = $(parent_id + '#arrival3').val(), 
		cat_id = $(parent_id + '#selectActivity').val(), 
		path = window.location.pathname, 
		code = $(parent_id + '#selectActivity option:selected').text(), 
		url, 
		dep = $(parent_id + '#departure3').val();
...
function search_rentals(parent_id) {
	
	parent_id = "#" + parent_id + " ";
	
    // rental - supercatid 6
    var arrival = $('#arrival4').val(), 
		cat_id = $('#selectRental').val(), 
		path = window.location.pathname, 
		url, 
		dep = $('#departure4').val(), 
		code = $('#selectRental option:selected').text();
...
function search_inpage_widget() {
	...
	$(".inpage-datepicker").datepicker({
		...
		onSelect: function (selectedDate) {
			...
			calculate_num_days("inpage-widget");
		}
	});
    $(".inpage-datepicker").datepicker().val(add_to_today(1).toString()).trigger('change');
	
    var catid = $('#acatid').val();
    $('#selectActivity').val(catid);
	
	calculate_num_days("inpage-widget");
	$(".widgetInPage select, .widgetInPage .inpage-datepicker, .widgetInPage .nights").uniform();
	
	/*if($("#inpage-widget").hasClass("dealswidget")) {
		setTimeout("fix_nights()", 1000);
		$("#inpage-widget #nights").val("1");
	}*/
...
function search_activity() {
    // activity - supercatid 2
    var arrival = $('#activity_search .arrival').val(), 
		departure = $('#activity_search .departure').val(), 
		catid = $('#activity_search #selectActivity').val(), 
		path = window.location.pathname, 
		url, 
		code = $('#activity_search #selectActivity option:selected').text();
    ...
    //       _gaq.push(['_link', url]);
  //  _gaq.push(['_trackEvent', 'activity', 'search', path], ['_link', url]);
_gaq.push(['_trackEvent', 'search activity', path, code], ['_link', url]);
...
function openSearchSuite() {

	var arrival = $('.widgetInPage #deals-arrival').val(), 
		dep = $('.widgetInPage #departure').val(), 
		persons = $('.widgetInPage #adults').val(), 
		nights = $('.widgetInPage #nights').val(), 
		a = new Date(arrival);
	...
	var mm1 = a.getMonth() + 1, 
		...
		yy1 = a.getFullYear(), 
		dep = mm1 + "/" + dd1 + "/" + yy1, 
		iChildCount = $('.widgetInPage #children').val(), 
		url;
	...
	var path = window.location.pathname;
	_gaq.push(['_trackEvent', 'search accomm', 'suitesecrets', path], ['_link', url]);
    ...
    if (iChildCount == 0) {
        ...
        url = 'http://www.inntopia.travel/aspnet/09/search.aspx?salesid=1321967&arrivaldate=' + arrival +
               ...
               '&NeighbourhoodId=' + neighbour +
                '&childcount=' + iChildCount +
                '&childagearray=' + create_child_array(iChildCount) + feature2;
    }
    _gaq.push(['_link', url]);
...
function openSearchAccom() {
    ...
    var hname = $('#hname').val();
    var arrival = $('#accommodation-arrival').val();
    var dep = $('#accommodation-departure').val();
    var persons = $('#inpage-widget #adults').val();
    var nights = $('#inpage-widget #nights').val();
    var a = new Date(arrival);
    a.setDate(a.getDate() + parseInt(nights));
    var mm1 = a.getMonth() + 1;
    var dd1 = a.getDate();
    var yy1 = a.getFullYear();
    var dep = mm1 + "/" + dd1 + "/" + yy1;
    var iChildCount = $('#inpage-widget #children').val();
    var url;
    ...
    _gaq.push(['_trackEvent', 'search a-z', 'search', hname],['_link', url]);
...
function openSearch() {
    ...
    var arrival = $('#search-all #arrival').val();
    var dep = $('#search-all #departure').val();
    var persons = $('#search-all #adults').val();
    var nights = $('#search-all #nights').val();
    var a = new Date(arrival);
    a.setDate(a.getDate() + parseInt(nights));
    var mm1 = a.getMonth() + 1;
    var dd1 = a.getDate();
    var yy1 = a.getFullYear();
    var dep = mm1 + "/" + dd1 + "/" + yy1;
    var iChildCount = $('#search-all #children').val();
    var url;
    if (iChildCount == 0) {
        url = 'http://www.inntopia.travel/aspnet/09/search.aspx?salesid=1321967&arrivaldate=' + arrival +
                '&departuredate=' + dep +
...
function openSearchAccom_Dog() {
    ...
    var catid = jQuery('#property').val();
    var code = jQuery('#property option:selected').text();
    var path = window.location.pathname;
    ...
    _gaq.push(['_trackEvent', 'search accomm', path, code], ['_link', url]);
